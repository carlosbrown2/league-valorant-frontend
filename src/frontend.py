...
def get_kds(df_player, wavg_col):
    ...
    kd_weightavg = df_player_kd.groupby("name").apply(lambda x: np.average(x["K/D"], weights=x["weight"]))
    kd_weightavg = kd_weightavg.reset_index().rename(columns={0: wavg_col})
    kd_pivot = df_player_kd.pivot_table(values="K/D", columns="agent", index="name").fillna(0).reset_index()
